...
```{r fig6, eval = TRUE, echo = FALSE, results = "hide", warning = TRUE, error = TRUE, message = FALSE, fig.width  = 1.25*fig.width, fig.height = phi*fig.height}
bowling.df %>% ggplot(aes(x=Date,y=Score))  +  geom_smooth(aes(color = Bowler), size =  1.5) +  geom_point(aes(color = Bowler), size =  1) +coord_cartesian(ylim = c(120, 180))  + theme_bw()
```
